...
UNION
select distinct 'create synonym ESINGL01.'||a.object_name||' for '|| a.owner ||'.'||a.object_name||';'
from dba_objects a
where to_char(created,'yy-mm-dd') > '09-01-01'
and owner in ('ESINGLRP')--acrecestar outros RPs
and object_type in ('TABLE','VIEW','PROCEDURE','FUNCTION','PACKAGE','QUEUE')
and not exists (select 1 from dba_synonyms aa where aa.owner='ESINGL01' and aa.synonym_name=a.object_name)
/
spo off
spo sintra_prd.log
@sintra_prd
